...
proc gL_DeleteContext*(context: GLContext): bool
proc gL_DestroyContext*(context: GLContext)
```
...
let win = createWindow("OpenGL", 800, 600, 0x00000002u64) # OPENGL flag
let ctx = gL_CreateContext(win)
...
```nim
discard renderer.setRenderDrawColor(30, 30, 30, 255)
discard renderer.renderClear()
discard renderer.setRenderDrawColor(255, 80, 0, 255)
discard renderer.renderFillRect(FRect(x: 100, y: 100, w: 200, h: 150))
...
**Пример RTT:**
```nim
let target = renderer.createTexture(PIXELFORMAT_RGBA8888,
                                    TEXTUREACCESS_TARGET, 512, 512)
discard renderer.setRenderTarget(target)
# рисуем в target ...
discard renderer.setRenderTarget(nil)  # обратно на экран
discard renderer.renderTexture(target, nil, nil)
```
...
proc getTextureColorMod*(texture: Texture, r, g, b: uint8): bool
...
type
  Surface* {.bycopy.} = object
    ...
    pixels*: pointer   # указатель на пиксели (доступен после Lock)
    refcount*: cint
```
...
```nim
var spec: AudioSpec
var audioBuf: ptr[uint8]
var audioLen: uint32

if not loadWAV("sound.wav", spec.addr, audioBuf, audioLen):
  echo getError()
  
let devid = openAudioDevice(0xFFFFFFFF'u32, nil) # SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK
let stream = openAudioDeviceStream(devid, spec.addr, nil, nil)
discard stream.putAudioStreamData(audioBuf, audioLen.cint)
discard stream.resumeAudioStreamDevice()
```
...
if count > 0:
  let pad = openGamepad(pads[0])
  let x = getGamepadAxis(pad, GAMEPAD_AXIS_LEFTX)
  let fire = getGamepadButton(pad, GAMEPAD_BUTTON_SOUTH)
  closeGamepad(pad)
```
...
proc acquireGPUSwapchainTexture*(command_buffer: GPUCommandBuffer,
                                 window: Window, swapchain_texture: GPUTexture,
                                 swapchain_texture_width: var uint32,
                                 swapchain_texture_height: var uint32): bool
proc waitAndAcquireGPUSwapchainTexture*(command_buffer: GPUCommandBuffer, ...): bool
...
discard props.setStringProperty(PROP_RENDERER_CREATE_NAME_STRING, "vulkan")
discard props.setNumberProperty(PROP_RENDERER_CREATE_PRESENT_VSYNC_NUMBER, 1)
    ...
    while pollEvent(event):
      ...
      of EVENT_KEY_DOWN:
        if event.key.scancode == SCANCODE_ESCAPE:
          ...
          discard window.setWindowFullscreen(
            (window.getWindowFlags() and 0x00000001u64) == 0)
      of EVENT_WINDOW_RESIZED:
        discard renderer.setRenderLogicalPresentation(
          800, 600, LOGICAL_PRESENTATION_LETTERBOX)
      else: discard

    # --- Рендеринг ---
    discard renderer.setRenderDrawColor(20, 20, 40, 255)
    discard renderer.renderClear()

    # Синий прямоугольник
    discard renderer.setRenderDrawColor(0, 120, 215, 255)
    discard renderer.renderFillRect(FRect(x: 100, y: 100, w: 200, h: 150))

    # Белая рамка
    discard renderer.setRenderDrawColor(255, 255, 255, 200)
    discard renderer.renderRect(FRect(x: 100, y: 100, w: 200, h: 150))

    # Диагональ
    discard renderer.setRenderDrawColor(255, 80, 0, 255)
    discard renderer.renderLine(0, 0, 800, 600)

    # Отладочный текст (встроенный 8×8 шрифт)
    discard renderer.setRenderDrawColor(255, 255, 0, 255)
    discard renderer.renderDebugTextFormat(10, 10, "FPS: %llu", frames)
...
> **Правило именования:** `SDL_` prefix убирается, первая буква становится строчной. `SDL_GetError` → `getError`. Исключения: функции OpenGL (`gL_*`), EGL (`eGL_*`), GPU (`gPU*`), HID (`hid_*`).
